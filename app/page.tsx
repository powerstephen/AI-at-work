...
function suggestedHours(team: Team, maturity: number) {
  // For maturity 1, suggest ~5 hrs/wk; for 10, ~1 hr/wk. Adjust per team.
...
function maturityDetail(score: number) {
  ...
  if (v <= 4) return 'Isolated champions; add templates, shared prompt lib, QA gates.';
...
function Calculator() {
  ...
  // Auto-suggest hours from maturity+team until user overrides
  useEffect(() => {
  ...
  // Styles
  const container = { maxWidth: 1120, margin: '0 auto', padding: '24px 20px 32px' } as const;
  const hero = {
  ...
  const help = { fontSize: '.86rem', color: '#E6ECFF' } as const;
  ...
  return (
    <main style={container}>
      {/* Blue hero with title + “What the report shows” in one band */}
      <section style={hero}>
        ...
        <div style={{ display: 'grid', gap: 10, gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))' }}>
          <div style={heroStat}>Monthly savings</div>
          <div style={heroStat}>Payback (months)</div>
          <div style={heroStat}>Annual ROI (×)</div>
          <div style={heroStat}>Hours saved / year</div>
        </div>
      </section>
      ...
      {step === 4 && (
        <section style={card}>
          <h3 style={{ ...h3, color: '#0F172A' }}>Training & Duration</h3>
          <div style={gridAuto}>
      ...
      {/* Build stamp to confirm fresh deploy */}
